...
export default function AddMenuItemScreen() {
  ...
  const [imageUri, setImageUri] = useState(null); // local file URI only — uploaded after item creation
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      // Just store locally — will upload after item is created
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Step 2: Upload image to the created item if one was selected
      const newItem = res.data.data?.menuItem || res.data.data;
      if (imageUri && newItem?.id) {
        try {
          await uploadMenuItemImage(restaurantId, newItem.id, imageUri);
        } catch {
          // Item was created — just warn about image
