...
function parseTime(hhmm: string | undefined): Date {
...
function formatDisplay(hhmm: string | undefined): string {
  if (!hhmm) return '--:--';
...
export default function TimePicker({ label, value, onChange, error }: TimePickerProps): React.JSX.Element {
  const [show, setShow] = useState<boolean>(false);
  const date = parseTime(value);

  const handleChange = (_: unknown, selected: Date | undefined): void => {
