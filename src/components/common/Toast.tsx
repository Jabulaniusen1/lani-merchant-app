...
let _showToast: ((options: ToastOptions) => void) | null = null;
...
export default function ToastProvider({ children }: ToastProviderProps): React.JSX.Element {
  ...
  const show = useCallback(
    ({ type = 'info', message }: ToastOptions): void => {
  ...
  return (
    <>
      ...
      {toast && config && (
        ...
        >
          <Ionicons name={config.icon as 'checkmark-circle'} size={20} color={config.iconColor} />
